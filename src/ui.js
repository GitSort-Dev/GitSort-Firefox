// ui.js — GitSort toolbar component matching GitHub's design system (Firefox)
...
var GitSortUI = (function () {
	...
	function createClockSvg() {
		...
		svg.setAttribute('class', 'gitsort-icon');
	...
	function updateAbsTimeButtonText(showAbsolute) {
		if (btnAbsTime) {
			...
			while (btnAbsTime.firstChild) {
				btnAbsTime.removeChild(btnAbsTime.firstChild);
			}

			btnAbsTime.appendChild(createClockSvg());

			const textNode = document.createTextNode(
				showAbsolute ? ' Timestamp' : ' Relative',
			);
			btnAbsTime.appendChild(textNode);
		}
	}
